...
namespace mrw
{
	namespace statechart
	{
		...
		void CrossingStatechart::raiseTimeEvent(sc::eventid evid)
		{
			...
			{
				incomingEventQueue.push_back(std::unique_ptr< EventInstance>(new EventInstance(static_cast<mrw::statechart::CrossingStatechart::Event>(evid + static_cast<sc::integer>(mrw::statechart::CrossingStatechart::Event::_te0_main_region_Operating_Processing_Pending_)))));
				runCycle();
			}
		}
...
// implementations of all internal functions
		...
		{
			/* Entry action for state 'Delay'. */
			timerService->setTimer(shared_from_this(), 1, (static_cast<sc::time> (CrossingStatechart::delay)), false);
		}
